...
PIPELINE=http://localhost:8888/dos-static-2013-02-15
...
#######
##DONE with the generation this should now be a working site at http://heuristscholar.org/dos-static-2013-02-15
#######ssh
...
rsync -av recaptcha kimj@dos-web-prd-1.ucc.usyd.edu.au:/var/www/dos-2013-02-15/

##########################
## on production server:##
##########################
cd /var/www/dos-2013-02-15
#replace dynamic links on built version to the relative links for the static production version
# old way gets Arg list too long error		perl -pi -e 's/http:\/\/heuristscholar.org\/dos-static-2013-02-15/../' item/*
grep -rl dos-static-2013-02-15 item |\
while read filename; do
	perl -pi -e 's/http:\/\/heuristscholar.org\/dos-static-2013-02-15/../' $filename;
done

# old way gets Arg list too long error		perl -pi -e 's/http:\/\/heuristscholar.org\/dos-static-2013-02-15/../' popup/*
grep -rl dos-static-2013-02-15 popup |\
while read filename; do
	perl -pi -e 's/http:\/\/heuristscholar.org\/dos-static-2013-02-15/../' $filename;
done
...
while read filename; do
	perl -pi -e 's/http:\/\/heuristscholar.org\/dos-static-2013-02-15/http:\/\/dictionaryofsydney.org/' $filename;
done
...
##########################################################################################
ln -s /var/www/dos-2013-02-15 test
