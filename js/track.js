  	...
  	HAPI.PJ.retrieve(_nameTrack, function (name, value){
		if (value.length > 1) {
			...
			if (value) {
				...
				for (var i = 1; i < (value.length < 6 ? value.length : 6); ++i){ //5 breadcrumbs for now, crumb[0] is current crumb
					var step = value[i];
					var a = document.createElement("a");
						a.href="../" + step.recId;				
						a.style.paddingBottom = "10px";
						a.style.textDecoration = "none";
					var t = document.createTextNode(step.recTitle);
					a.appendChild(t);
					...
					if (step.hasGeoData &&  step.hasGeoData != ""){
