...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- This style sheet is based on a copy of item_view.xsl for the purposes of developing a production
	prototye for late 08 DoS work 
	
	Refer to Steven for furhter details
	-->
	...
	<xsl:variable name="browserbase">/prototype</xsl:variable>
	...
	<xsl:include href="entity_prototype.xsl"/>
	...
	<xsl:template match="/">
		...
		<html>
			<head>
				<title>
					<xsl:value-of select="export/references/reference/title"/> (prototype viewer)
				</title>
				<!-- structural styling -->
				<style type="text/css">
				...
				<script>
					var pathDos = "http://heuristscholar.org/cocoon/dos/sandbox/prototype/";
					...
					function annotation_loaded(record) {
						...
						elts.innerHTML = record.getTitle();
			...
			<body pub_id="{/export/@pub_id}"  >
				...
				<div id="header">
					...
					<div id="logo">
						<a href="{$urlbase}/index.html" style="font-size: 30px;">PROTOTYPE</a>
					</div>
					...
					<div id="sidebartopcolour" class="colourcellfour">
					<table width="100%">
					<tr>
					<td id="login">
					...
					<td>
						<xsl:if test="detail[@id = 222 or @id=223 or @id=224]">
							<xsl:if test="detail/file_thumb_url">
								<a href="{$cocoonbase}{$browserbase}/{id}">
						...
						<a href="{$cocoonbase}{$browserbase}/{id}" class="sb_two">
							<xsl:choose>
								<!-- related / notes -->
								<xsl:when test="@notes">
