...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="factoids">
		<xsl:if test="count(reversePointer/record[reftype/@id=150][detail[@id=526]='Type']) > 1">
			<xsl:call-template name="factoidGroup">
				<xsl:with-param name="heading">Types</xsl:with-param>
				<xsl:with-param name="factoids" select="reversePointer/record[reftype/@id=150][detail[@id=526]='Type']"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:call-template name="factoidGroup">
			<xsl:with-param name="heading">Names</xsl:with-param>
			<xsl:with-param name="factoids" select="reversePointer/record[reftype/@id=150][detail[@id=526]='Name']"/>
		</xsl:call-template>
		<xsl:call-template name="factoidGroup">
			<xsl:with-param name="heading">Milestones</xsl:with-param>
			<xsl:with-param name="factoids" select="reversePointer/record[reftype/@id=150][detail[@id=526]='Milestone']"/>
		</xsl:call-template>
		<xsl:call-template name="factoidGroup">
			<xsl:with-param name="heading">Relationships</xsl:with-param>
			<xsl:with-param name="factoids" select="reversePointer/record[reftype/@id=150][detail[@id=526]='Relationship']"/>
		</xsl:call-template>
		<xsl:call-template name="factoidGroup">
			<xsl:with-param name="heading">Occupations</xsl:with-param>
			<xsl:with-param name="factoids" select="reversePointer/record[reftype/@id=150][detail[@id=526]='Occupation']"/>
		</xsl:call-template>
		<xsl:call-template name="factoidGroup">
			<xsl:with-param name="heading">Positions</xsl:with-param>
			<xsl:with-param name="factoids" select="reversePointer/record[reftype/@id=150][detail[@id=526]='Position']"/>
		</xsl:call-template>
	</xsl:template>
